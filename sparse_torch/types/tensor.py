...
@dataclass(frozen=True)
class LevelFormat:
    level_format: Optional[Format] = None
...
class Tensor[T](Ref[TensorStorage, "Tensor[T]"]):
    ...
    def new(format: List[Format], shape: Tuple[int, ...]) -> "Tensor[T]":
        return Tensor().const(TensorStorage(TensorFormat([LevelFormat(form) for form in format]), shape))
    ...
    def __matmul__(self, other: "Tensor[T]") -> "Tensor[T]":
        ...
        assert k0 == k1, f"Invalid shapes for matrix multiplication with expected signature: (n,k),(k,m)->(n,m) -- actual shape: ({
            m}, {k0}), ({k1}, {n})"
        return stage(sparse_torch.Matmul[Tensor[T := TensorStorage(tensor_format=other.get_format(), shape=(self.get_shape()[0], other.get_shape()[1]))]](self, other), ctx=SrcCtx.new(2))
    ...
    def reduce(self, reduceDim=-1):
        return stage(sparse_torch.Reduce[Tensor[T := TensorStorage(tensor_format=self.get_format(), shape=self.get_shape()[:1]+(1,))]](self, reduceDim))

    def max_reduce(self, reduceDim=-1):
        return stage(sparse_torch.MaxReduce[Tensor[T := TensorStorage(tensor_format=self.get_format(), shape=self.get_shape()[:1]+(1,))]](self, reduceDim))
